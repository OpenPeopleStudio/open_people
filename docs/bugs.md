...
| BUG-002 | ESLint error: hook uses function before declaration | High | Super Admin UI | Lisa | Triage | `npm run lint` | Expected: lint clean; Actual: `react-hooks/immutability` error | `components/super-admin/email-campaigns/CampaignsClient.tsx` | Lint still fails 2026-01-22: `loadCompanies/loadGroups/loadCampaigns/loadAccounts` used before declaration |
