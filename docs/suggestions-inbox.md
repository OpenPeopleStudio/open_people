...
- Add suggestions when you see an improvement or risk that is not already in `docs/TODO.md`.
- Use the template at the bottom; include owner and file targets when possible.
- Tag with categories; include tenant/safety impact if applicable.
- Do not delete suggestions. Mark as addressed once implemented.

**For Developers:**

- Triage weekly; promote the best items into `docs/TODO.md` with owner, files, DoD.
- Link tasks/decisions back to the suggestion entry.
- If a suggestion is blocked, record the blocker and keep it active.

**For Company Ops:**

- CEO/CTO scan monthly to shape roadmap themes.
- Coder converts accepted items into tasks and coordinates owners.
- Employees can propose and refine items but do not change status without Coder/CTO.

**Legacy note:** Older entries may not include all fields; backfill missing metadata when touched.

---

## Lifecycle + Status

- **New:** Added, not yet triaged.
- **Triaged:** Reviewed; has owner and next action.
- **Queued:** Accepted for roadmap but not yet scheduled.
- **Doing:** Active implementation (must link to TODO task).
- **Blocked:** Waiting on dependency or decision (state reason).
- **Addressed:** Implemented and verified (link to task/PR/ADR).
- **Parked:** Low value or superseded; keep for record.

---

## Triage Cadence

- **Weekly (Coder + CTO):** Triage new entries, assign owners, promote to TODO or ADR.
- **Monthly (CEO + CTO):** Review themes and adjust roadmap focus.
- **Quarterly (Coder):** Prune duplicates, merge overlaps, and mark parked items.

---

## Promotion Rules

- Move to `docs/TODO.md` only when owner, files, and DoD are clear.
- If scope changes architecture or adds dependencies, attach an ADR first.
- Any suggestion touching auth/tenant isolation must include Mr Robot review.

---

## Scoring Rubric (lightweight)

- **Impact:** User or business value (1-5).
- **Effort:** Implementation size (1-5).
- **Risk:** Tenant/security/operational risk (1-5).
- **Confidence:** Evidence strength (1-5).

---

## Linking Rules

- **TODO:** Add task link when status becomes Doing or Addressed.
- **ADR:** Link an ADR when the suggestion changes architecture or dependencies.
- **Coordination:** Log handoffs in `docs/company/coordination.md`.

---

### [DEVEX] Repo Utility Script Pack

**Priority:** Medium | **Added:** 2026-01-24 | **Status:** New
**Owner:** Coder

**Context:** Common tasks (lint fixes, typecheck, tests, and doc formatting) are invoked manually and inconsistently.

**Recommendation:**

1. Add `scripts/run-standard-checks.sh` that runs lint → typecheck → unit tests with clear exit codes.
2. Add `scripts/fix-lint.sh` to run `eslint --fix` on staged files.
3. Add `scripts/format-docs.sh` to normalize Markdown linting and link checks.

**Files:** `scripts/run-standard-checks.sh`, `scripts/fix-lint.sh`, `scripts/format-docs.sh`, `package.json`
**Tenant/Safety Impact:** None
**Links:** TODO / ADR / PR (if any)

**Impact:** Faster, more consistent local checks with fewer context-switches.

---

### [DATA-OPS] Supabase Reset + Seed Helper

**Priority:** Medium | **Added:** 2026-01-24 | **Status:** New
**Owner:** Coder

**Context:** Local environments can drift; repeated reset/seed steps are easy to miss.

**Recommendation:**

1. Add `scripts/db-reset.sh` to stop/start local Supabase, reset DB, run migrations, and seed Mars tenant.
2. Add `npm run db:reset` script that calls the helper and prints next steps.

**Files:** `scripts/db-reset.sh`, `package.json`, `docs/development/setup.md`
**Tenant/Safety Impact:** Low
**Links:** TODO / ADR / PR (if any)

**Impact:** Reliable local reset flow and faster onboarding.

---

### [DEVEX] Workspace Sanity Snapshot

**Priority:** Low | **Added:** 2026-01-24 | **Status:** New
**Owner:** Coder

**Context:** Debugging local issues often starts with asking for the same environment details.

**Recommendation:**

1. Add `scripts/workspace-snapshot.sh` to capture node version, npm version, OS, git status, and active env vars (redacted).
2. Emit a clean, shareable report file for support/coordination logs.

**Files:** `scripts/workspace-snapshot.sh`, `docs/support/troubleshooting.md`
**Tenant/Safety Impact:** Low
**Links:** TODO / ADR / PR (if any)

**Impact:** Faster triage and reproducible debug reports.
...
**Priority:** High / Medium / Low | **Added:** YYYY-MM-DD | **Status:** New / Triaged / Queued / Doing / Blocked / Addressed / Parked
**Owner:** <Role>
...
**Files:** Suggested files or areas (if known)
**Tenant/Safety Impact:** None / Low / Medium / High
**Links:** TODO / ADR / PR / Coordination entry (if any)

**Impact:** Why does this matter?
```

**Categories:** `SECURITY`, `ARCHITECTURE`, `PERFORMANCE`, `UX`, `FEATURE`, `DEVEX`, `TESTING`, `MONITORING`, `DOCS`, `DATA-OPS`
