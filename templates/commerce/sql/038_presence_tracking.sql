-- Add presence tracking fields to profiles
...
BEGIN
  IF EXISTS (
    ...
    CREATE INDEX IF NOT EXISTS idx_profiles_presence 
      ON "profiles" (tenant_id, is_online, last_active_at DESC);
  ELSE
    CREATE INDEX IF NOT EXISTS idx_profiles_presence 
      ON "profiles" (is_online, last_active_at DESC);
  END IF;
END $$;
...
COMMENT ON COLUMN "profiles".last_active_at IS 'Last time the user sent a presence heartbeat (updated every 60s)';
COMMENT ON COLUMN "profiles".is_online IS 'True if user has been active within last 5 minutes';
