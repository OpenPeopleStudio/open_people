...
DROP POLICY IF EXISTS "Users can update own public key" ON "profiles";
CREATE POLICY "Users can update own public key" ON "profiles"
  FOR UPDATE USING (auth.uid() = id)
  WITH CHECK (auth.uid() = id);

-- Allow reading public keys (needed for encryption)
DROP POLICY IF EXISTS "Anyone can read public keys" ON "profiles";
CREATE POLICY "Anyone can read public keys" ON "profiles"
...
COMMENT ON COLUMN "profiles".public_key IS 'User ECDH P-256 identity public key for E2EE (SPKI format, base64)';
