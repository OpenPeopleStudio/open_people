  ...
  return (
    ...
    >
      <svg viewBox={`0 0 ${W} ${H}`} role="img" aria-labelledby="gc-title gc-desc">
        ...
        <desc id="gc-desc">
          A time rail from 17 August 2026, when the framework was signed, to 31 March 2027, when its
          term ends. Gates: House vote 17 September, Québec election 5 October, binding target 31
          December, term end 31 March. First Gull Island power is pencilled for 2036, off the scale.
        </desc>
        <defs>
          <pattern id="gc-hatch" width="6" height="6" patternUnits="userSpaceOnUse" patternTransform="rotate(45)">
            <line x1="0" y1="0" x2="0" y2="6" stroke="var(--steel)" strokeWidth="1" />
          </pattern>
        </defs>

        {/* rail */}
        <line x1={PAD} x2={PAD + ON_SCALE_W} y1={RAIL_Y} y2={RAIL_Y} className="hair draw" pathLength={1} />
        {/* break to off-scale */}
        <path
        ...
        {onScale.map((g, i) => {
          ...
          const above = i % 2 === 0;
          const cls =
            g.status === "open" ? "amber" : g.status === "endorsed" ? "plasma" : "plasma";
          return (
            <g key={g.id} transform={`translate(${gx} ${RAIL_Y})`}>
              <line y1={above ? -26 : 0} y2={above ? 0 : 26} className="hair" />
              <circle r={4.5} className={cls} />
              {g.kind === "target" ? (
                <circle r={9} fill="none" className={`stroke-${cls}`} strokeDasharray="2 3" strokeWidth={1} />
              ) : null}
              <text y={above ? -36 : 44} textAnchor="middle" className="num" style={{ fontSize: 14 }}>
                {g.short}
              </text>
        ...
        {off.map((g) => (
          <g key={g.id} transform={`translate(${x(g)} ${RAIL_Y})`}>
            <rect x={-9} y={-9} width={18} height={18} fill="url(#gc-hatch)" stroke="var(--steel)" strokeWidth={1} />
            <text y={-36} textAnchor="middle" className="num" style={{ fontSize: 14 }}>
              {g.short}
            </text>
            <text y={-52} textAnchor="middle" className="lbl">
              first Gull Island power
            </text>
            <text y={44} textAnchor="middle" className="lbl steel" style={{ fontSize: 9.5 }}>
              preliminary · Annex B
            </text>
          </g>
        ))}

        <TodayMarker pad={PAD} width={ON_SCALE_W} y1={RAIL_Y - 70} y2={RAIL_Y + 66} />
      </svg>
      ...
      <p className="inst-caption">
        Solid dots are fixed dates. Dashed rings are stated targets in the DCIA text. The hatched
        square is a preliminary Annex B year. Nothing on this rail is a forecast by Open People.{" "}
        ...
        {GATES.map((g) => (
          <li key={g.id} className="inst-legend-item">
            <span
              ...
              data-fill={g.kind === "preliminary" ? "hatch" : g.kind === "target" ? "outline" : "solid"}
            />
            <span>
              <Figure
                value={g.short}
