...
import { DESK_SOURCES, LADDER_FAMILIES, LADDER_POINTS, type LadderFamily } from "@/lib/desk";
...
export function PriceLadder() {
  ...
  const reported = LADDER_POINTS.filter((p) => p.family === "reported" && p.cents !== null).sort(
  ...
  return (
    <div className="inst price-ladder">
      <svg viewBox={`0 0 ${W} ${H}`} role="img" aria-labelledby="pl-title pl-desc">
        <title id="pl-title">The price ladder</title>
        <desc id="pl-desc">
          Cents per kilowatt-hour on one axis, with each price family on its own row: heritage
          export, the rejected 2024 MOU, the reported new Hydro-Québec export path, published Labrador
          tariffs, the Island grid, Québec&apos;s proposed data-centre rate, and the signed industrial
          rate for retained power, which is not published.
        </desc>
        ...
        {LADDER_FAMILIES.map((fam, i) => {
          ...
          return (
            <g key={fam.id}>
              ...
              <text x={LEFT - 14} y={y + 4} textAnchor="end" className="lbl lbl-strong">
                {fam.label}
              </text>
              {fam.id === "reported" && reported.length > 1 ? (
                <path
                  d={reported.map((p, k) => `${k === 0 ? "M" : "L"} ${cx(p.cents!)} ${y}`).join(" ")}
                  fill="none"
              ...
              {fam.id === "signed" ? (
                <g>
                  <rect
                    ...
                    strokeWidth={1}
                  />
                  <text x={(cx(2) + cx(12)) / 2} y={y + 4} textAnchor="middle" className="lbl alert">
                    not published
                  </text>
                </g>
              ) : null}
      ...
      <div className="ladder-rows" role="list" aria-label="Price ladder rungs">
        {LADDER_FAMILIES.map((fam) => {
          const pts = LADDER_POINTS.filter((p) => p.family === fam.id);
          return pts.map((p) => (
            <div key={p.id} className="ladder-row" role="listitem" id={`ladder-${p.id}`}>
              <div>
                <Figure
        ...
        <div className="ladder-row" role="listitem">
          ...
          <div className="d">
            The mine rate is demand charges plus a monthly formula, RFIRM = {"{"}(ED × RD) + (EM ×
            RM){"}"} / ETOTAL. It is not one cent and is not drawn as a rung.{" "}
      ...
      <p className="inst-caption">
        Each row measures something different. A starting price, a fifty-year average and a
        household tariff cannot be subtracted from one another. The desk labels them and refuses to
        merge them. Source for each rung is under its number.
      </p>
    </div>
  );
}
